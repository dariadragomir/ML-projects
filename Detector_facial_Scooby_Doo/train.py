...
max_f1 = 0.95
faces_fred = np.array([img for img in np.load(FACES_PATH + '/' + "fred.npy")])
faces_fred_zoomed_out = np.array([img for img in np.load(FACES_PATH + '/' + "fred_zoomed_out.npy")])

print(faces_fred.shape)
w, h, _ = faces_fred[0].shape
...
faces_unknown = np.array([cv.resize(img,(h, w)) for img in np.load(FACES_PATH + '/' + "unknown.npy")])
background = np.array([img for img in np.load(BACKGROUND_PATH + "fred_background.npy")])
faces_flip = np.flip(faces_fred, axis=2)

X = np.concatenate((faces_fred, faces_fred_zoomed_out, faces_daphne, faces_flip, faces_velma, faces_shaggy, faces_unknown, background))
y = np.concatenate(
    (np.array([1 for _ in range(len(faces_fred))]),
     np.array([1 for _ in range(len(faces_fred_zoomed_out))]),
     np.array([0 for _ in range(len(faces_daphne))]),
     np.ones(faces_flip.shape[0]),
     np.array([0 for _ in range(len(faces_velma))]),
     np.array([0 for _ in range(len(faces_shaggy))]),
     np.array([0 for _ in range(len(faces_unknown))]),
...
for epoch in tqdm(range(35)):
    ...
    with torch.no_grad():
        ...
        if avg_val_f1>max_f1:
            max_f1 = avg_val_f1
            torch.save(model, f"./Models/fred/CNNfred5sigmoid.pth")